...
class ProductPage(BasePage):
    ...
    def get_product_name(self):
        product_name = self.is_present_element(*ProductPageLocators.PRODUCT_NAME)
        if product_name:
            self.product_name = product_name.text
        else:
            assert False, 'No product name'

    def get_product_price(self):
        product_price = self.is_present_element(*ProductPageLocators.PRODUCT_PRICE)
        if product_price:
            self.product_price = product_price.text
        else:
            assert False, 'No product price'

    def should_be_product_added_message(self):
        product_added_message = self.browser.find_element(*ProductPageLocators.ADDED_MESSAGE).text
